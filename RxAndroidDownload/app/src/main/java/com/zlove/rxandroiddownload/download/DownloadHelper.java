...
public class DownloadHelper {
    ...
    Boolean retry(Integer integer, Throwable throwable) {
        if (throwable instanceof UnknownHostException) {
            ...
            if (integer < MAX_RETRY_COUNT + 1) {
                Log.w(TAG, TextUtils.concat(Thread.currentThread().getName(), " ", throwable.getMessage(),
            ...
            if (integer < MAX_RETRY_COUNT + 1) {
                Log.w(TAG, Thread.currentThread().getName() + " a network or conversion error happened, retry to connect " + integer + " times");
                return true;
            }
            return false;
